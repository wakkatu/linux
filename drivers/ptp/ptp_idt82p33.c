				 ...
				 u8 ref, bool enable)
{
	...
	if (enable) {
		/* Simply return if it was already enabled */
		...
		(void)idt82p33_get_extts(channel, &ts);
...
static u8 idt82p33_enable_extts_mask(struct idt82p33_channel *channel,
			     ...
			     struct timespec64 *ts)
{
	...
	if (idt82p33->extts_mask) {
		extts_mask = idt82p33_enable_extts_mask(channel, idt82p33->extts_mask,
						     false);
	}
...
{
	s64 scaled_ppb = (ddco_ppb << 13) / 125;
	s64 max = ( DCO_MAX_PPB << 13 ) / 125;

	current_ppm += scaled_ppb;

	if (scaled_ppb > max)
		scaled_ppb = max;
	else if (scaled_ppb < -max)
		scaled_ppb = -max;
