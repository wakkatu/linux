...
static inline int idtcm_read(struct idtcm *idtcm,
			     ...
			     u16 count)
{
	return rsmu_read(idtcm->mfd, module + regaddr, buf, count);
}
...
static inline int idtcm_write(struct idtcm *idtcm,
			      ...
			      u16 count)
{
	return rsmu_write(idtcm->mfd, module + regaddr, buf, count);
}
				     ...
				     enum scsr_read_trig_sel trig, u8 ref)
{
	...
	err = idtcm_read(idtcm, channel->tod_read_primary,
			 TOD_READ_PRIMARY_CMD, &val, sizeof(val));
	if (err)
		return err;
			      ...
			      bool enable)
{
	...
	if (old_mask == 0 && idtcm->extts_mask)
		schedule_delayed_work(&idtcm->extts_work, EXTTS_PERIOD);
	else if (idtcm->extts_mask == 0)
		cancel_delayed_work_sync(&idtcm->extts_work);

	return err;
}

static void idtcm_enable_extts_mask(struct idtcm_channel *channel,
				    ...
				    struct timespec64 *ts)
{
	...
	/* Disable extts */
	if (idtcm->extts_mask) {
		extts_mask = idtcm->extts_mask;
		idtcm_enable_extts_mask(channel, extts_mask, false);
	}
			       ...
			       struct device *dev)
{
	...
	dev_dbg(idtcm->dev, "requesting firmware '%s'", fname);
...
static void idtcm_extts_check(struct work_struct *work)
{
	...
	for (i = 0; i < MAX_TOD; i++) {
		...
		if (idtcm->extts_mask & mask) {
			...
			extts_channel = &idtcm->channel[i];
			ptp_channel = idtcm->event_channel[i];
			...
			if (err == 0) {
				...
				event.timestamp = timespec64_to_ns(&ts) -
						  dco_delay;
				ptp_clock_event(ptp_channel->ptp_clock,
						&event);
			}
		}
	}
...
static const struct platform_device_id idtcm_id_table[] = {
	{ "idtcm-ptp0", },
	{ "idtcm-ptp1", },
	{ "idtcm-ptp2", },
	{ "idtcm-ptp3", },
...
MODULE_DEVICE_TABLE(platform, idtcm_id_table);
